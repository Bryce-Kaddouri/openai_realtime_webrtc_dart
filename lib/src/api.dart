...
class RealtimeAPI extends RealtimeEventHandler {
  ...
  RealtimeAPI({
    this.url = 'https://api.openai.com/v1/realtime',
    this.apiKey,
    this.headers,
    this.debug = false,
  ...
  bool isConnected() => _peerConnection != null;
  ...
  Future<bool> connect({final String model = RealtimeUtils.defaultModel}) async {
    ...
    try {
      ...
      _dataChannel = webrtc.dataChannel;

      // Wait for the connection to be established
      await webrtc.waitUntilReady();
      ...
      _dataChannel!.onMessage = (RTCDataChannelMessage message) {
        if (message.isBinary) {
          ...
          final msg = json.decode(message.text) as Map<String, dynamic>;
          unawaited(receive(msg));
        }
      };
      ...
      _dataChannel!.onDataChannelState = (RTCDataChannelState state) {
        if (state == RTCDataChannelState.RTCDataChannelClosed) {
          unawaited(
            dispatch(RealtimeEventType.close, RealtimeEvent.close(eventId: RealtimeUtils.generateId(), error: false)),
          );
        }
      };
    ...
    void replaceAudioProperty(dynamic json) {
      if (json is Map<String, dynamic>) {
        json.forEach((key, value) {
          if (key == 'audio' || (key == 'delta' && json['type'] == 'response.audio.delta')) {
