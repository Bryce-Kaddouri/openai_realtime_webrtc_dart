...
class RealtimeClient extends RealtimeEventHandler {
  ...
  Future<void> _callTool(FormattedTool tool) async {
    try {
      ...
      final jsonArguments = jsonDecode(arguments) as Map<String, dynamic>? ?? const {};
  ...
  Future<void> updateSession({
    ...
    sessionConfig = sessionConfig.copyWith(
      ...
      inputAudioTranscription: inputAudioTranscription ?? sessionConfig.inputAudioTranscription,
      turnDetection: turnDetection == _turnDetectionSentinelValue ? sessionConfig.turnDetection : turnDetection,
      tools: useTools,
      toolChoice: toolChoice ?? sessionConfig.toolChoice,
      temperature: temperature ?? sessionConfig.temperature,
      maxResponseOutputTokens: maxResponseOutputTokens ?? sessionConfig.maxResponseOutputTokens,
    );
    ...
    if (id == null) {
      ...
      await realtime.send(
        RealtimeEvent.conversationItemTruncate(
          ...
          audioEndMs: (sampleCount / conversation.defaultFrequency * 1000).floor(),
        ),
      );
      return item;
    }
